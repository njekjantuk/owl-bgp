...
public class GTPAnnotationLiteralHandler extends AbstractLiteralTripleHandler {
    ...
    public boolean canHandleStreaming(Identifier subject, Identifier predicate, Literal object) {
        return !consumer.isAnonymous(subject) && !consumer.isAnnotation(subject) && consumer.isAnnotationProperty(predicate);
    }
    ...
    public void handleTriple(Identifier subject, Identifier predicate, Literal object) {
        ...
        if (consumer.isOntology(subject))
            ...
            consumer.addAxiom(AnnotationAssertion.create(consumer.translateAnnotationPropertyExpression(predicate), (AnnotationSubject)subject, (AnnotationValue)object, consumer.getPendingAnnotations()));
    }
}
