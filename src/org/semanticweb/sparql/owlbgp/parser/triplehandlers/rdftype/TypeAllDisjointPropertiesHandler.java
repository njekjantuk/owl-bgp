...
public class TypeAllDisjointPropertiesHandler extends BuiltInTypeHandler {
    ...
    public void handleTriple(Identifier subject, Identifier predicate, Identifier object) {
        ...
        if (consumer.isObjectPropertyOnly(first)) {
            ...
            consumer.addAxiom(DisjointObjectProperties.create(new HashSet<ObjectPropertyExpression>(props),consumer.getAnnotations(subject)));
        } else if (consumer.isDataPropertyOnly(first)) {
            List<DataPropertyExpression> props = consumer.translateToDataPropertyList(listNode);
            consumer.addAxiom(DisjointDataProperties.create(new HashSet<DataPropertyExpression>(props),consumer.getAnnotations(subject)));
        } else 
